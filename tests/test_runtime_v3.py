...
def test_control_sequence_artifact_without_causal_history_marker_is_rejected(
    tmp_path: Path,
) -> None:
    args = _artifact(tmp_path)
    manifest = json.loads(args[1].read_text(encoding="utf-8"))
    manifest["capabilities"].append("control_sequence")
    args[1].write_text(json.dumps(manifest, sort_keys=True), encoding="utf-8")
    with pytest.raises(ValueError, match="causal command history alignment"):
        load_runtime_model_v3(
            args[0],
            args[1],
            device=torch.device("cpu"),
