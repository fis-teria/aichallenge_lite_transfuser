...
def run_plant(policy: str, shape: str) -> tuple[float, float]:
    ...
    for step in range(2001):
        ...
        while delayed and delayed[0][0] < step-34:
            delayed.popleft()
        old_targets = [value for capture, value in delayed if capture <= step-14]
...
def test_lead_reduces_dynamic_physical_tracking_error(shape):
    baseline = run_plant(CALIBRATED_POLICY, shape)
    lead = run_plant(LEAD_POLICY, shape)
