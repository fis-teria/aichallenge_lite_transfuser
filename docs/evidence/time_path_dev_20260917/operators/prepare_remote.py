...
write('awsim_before.json',dict(files={p.relative_to(sim).as_posix():dict(bytes=p.stat().st_size,sha256=sha(p)) for p in sorted(sim.rglob('*')) if p.is_file()}))
...
assert (root/'command_off_best.pt').is_file(), 'verified WSL checkpoint must be copied first'
...
for p in (source/'ros2_ws/src/aic_e2e_runtime/aic_e2e_runtime').rglob('*.py'):
 ...
 assert sha(p)==sha(installed),str(rel);checked.append({'path':str(rel),'sha256':sha(installed)})
for relative, destination in [('ros2_ws/src/aic_e2e_runtime/launch/time_path_awsim.launch.py','launch/time_path_awsim.launch.py'),
                              ('configs/control/time_path_dev.json','config/time_path_dev.json')]:
 p=source/relative;installed=root/'install/aic_e2e_runtime/share/aic_e2e_runtime'/destination
 assert sha(p)==sha(installed),relative
 checked.append(dict(path=destination,sha256=sha(installed)))
...
# The deployment root is a stable user entry point. Recursive make propagates
# command-line speed variables and uses only this source-verified release.
makefile = '# Prepared TimePath deployment; source '+manifest['source_commit']+'\n.PHONY: dev\ndev:\n\t$(MAKE) -C "$(dir $(abspath $(lastword $(MAKEFILE_LIST))))'+source.name+'" dev DEV_CONTROLLER=time\n'
with (root/'Makefile').open('x') as stream:
    stream.write(makefile)
write('dev_entrypoint.json',dict(source_commit=manifest['source_commit'],makefile_sha256=sha(root/'Makefile'),
    command='make dev MAX_SPEED_KMH=20 CORNER_MAX_SPEED_KMH=10'))
