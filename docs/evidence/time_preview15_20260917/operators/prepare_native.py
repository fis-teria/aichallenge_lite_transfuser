...
out=base/'runs/time_preview15_20260917';out.mkdir(exist_ok=False)
...
cfg_path=repo/'configs/control/time_path_timepreview_15kmh_20260917.json'
control=json.loads(cfg_path.read_bytes());old=json.loads((repo/'configs/control/time_path_curvature_logonly_20260917.json').read_bytes())
assert {k for k in control.keys()|old.keys() if control.get(k)!=old.get(k)}=={'speed_policy','lookahead_policy'}
assert validate_trial_config(control)=='curvature_time_preview_15kmh_v1' and control['checkpoint_sha256']==digest
...
record=dict(status='PASS',exit=0,source_commit=head,tested_source_commit=head,
    ...
    runtime_contract_valid=True,predictions=rows,runtime_speed_profile_changed=True,time_preview_alignment=True,scan_proximity_log_only=True,stopping_distance_diagnostic=True,sealed_test_read=False,
    offline_selection_sha256=hashlib.sha256((prior/'evaluation_final/selection.json').read_bytes()).hexdigest(),
    offline_selection_runtime_test_allowed=False,user_authorization='User requested alignment of time prediction and PP preview, retaining the previous log-only proximity experiment; compare 15 km/h first then 20 km/h',
