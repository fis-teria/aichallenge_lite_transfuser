"""Owned separated recovery campaign with explicit finite replacement amendment."""
...
ROOT = '/home/graneple/e2e_autonomous/time_recovery_separated_20260915'
OUT = '/home/thistle/e2e_autonomous/runs/time_recovery_separated_20260915'
RAW = '/home/thistle/e2e_autonomous/raw/time_recovery_separated_20260915'
UNC = Path(r'\\wsl.localhost\Ubuntu-22.04-Recovered\home\thistle\e2e_autonomous\runs\time_recovery_separated_20260915')
...
def setup() -> None:
    ...
    prep = dict(commit=head, archive_sha256=hashlib.sha256(archive.read_bytes()).hexdigest(), test_gate=gate,
                scope='SEPARATED_S00_PILOT_THEN_TEN_SITES', maximum_attempts=10, maximum_pulse_events=22)
...
shutil.copy2(root.parent/'time_recovery_sites_20260915'/'display.json',root/'display.json')
...
if (root/'collection_pause.json').exists():raise RuntimeError('COLLECTION_PAUSED_FOR_LOCATION_DURATION_REVIEW')
ledger=json.loads((root/'campaign_20260914.json').read_text())
amendment=None
if ledger.get('replacement_amendment_sha256'):
 amendment_bytes=(root/'replacement_amendment.json').read_bytes()
 assert hashlib.sha256(amendment_bytes).hexdigest()==ledger['replacement_amendment_sha256']
 amendment=json.loads(amendment_bytes)
 assert amendment['maximum_attempts']==ledger['maximum_attempts']==11
 assert amendment['maximum_pulse_events']==ledger['maximum_pulse_events']==22
 assert amendment['failed_run_id']=='codex-time-recovery-separated-g05-left'
 assert amendment['replacement_run_id']=='codex-time-recovery-separated-g05-left-r1'
 assert amendment['original_selection_sha256']==ledger['selection_sha256']
else:assert ledger['maximum_attempts']==10
assert not ledger['sealed'] and len(ledger['attempts'])<ledger['maximum_attempts']
row=ledger['planned_runs'][len(ledger['attempts'])];assert row['run_id']==run_id
if row['pair']>1:
 gate=json.loads((root/'pilot_gate.json').read_text());assert gate['expansion_allowed'] and gate['plan_sha256']==ledger['selection_sha256']
...
for previous in ledger['attempts']:
 ...
 if amendment and previous['run_id']==amendment['failed_run_id']:
  assert previous['state']=='WSL_MOVED'
  digest=hashlib.sha256(json.dumps(r,sort_keys=True,separators=(',',':')).encode()).hexdigest()
  assert digest==amendment['failed_result_canonical_sha256']
  assert r['status']=='STOPPED_FAILED' and r['last_control']['fault']=='COLLECTION_COMPUTATION_TIMEOUT'
  assert r['last_control']['random_pulse']['state']['event_id']==0 and r['last_control']['stop_confirmed']
  assert r['nodes']['closed_bag'] and not r['cleanup_errors']
 else:assert r['status']=='COMPLETE_LAP' and not r['last_control']['fault'] and r['nodes']['closed_bag'] and not r['cleanup_errors']
...
def ship(pair: int, *, prefix: str | None = None) -> None:
    prefix=prefix or f'separated_pair{pair:02d}_20260915'
...
excluded=set(ledger.get('excluded_failed_runs',[]))
print(json.dumps(dict(planned=[r for r in ledger['planned_runs'] if r['pair']=={pair} and r['run_id'] not in excluded],attempted=[r['run_id'] for r in ledger['attempts']])))
'''))
