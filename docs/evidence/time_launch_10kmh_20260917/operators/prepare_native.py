...
out=base/'runs/time_launch_10kmh_20260917';out.mkdir(exist_ok=False)
...
cfg_path=repo/'configs/control/time_path_launch_10kmh_20260917.json'
control=json.loads(cfg_path.read_bytes());old=json.loads((repo/'configs/control/time_path_launch_probe_20260917.json').read_bytes())
...
record=dict(status='PASS',exit=0,source_commit=head,tested_source_commit=head,
    ...
    runtime_contract_valid=True,predictions=rows,runtime_speed_profile_changed=True,sealed_test_read=False,
    offline_selection_sha256=hashlib.sha256((prior/'evaluation_final/selection.json').read_bytes()).hexdigest(),
    offline_selection_runtime_test_allowed=False,user_authorization='target 10 km/h AWSIM run requested 2026-09-17',
