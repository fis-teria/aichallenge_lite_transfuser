...
root = Path('/home/graneple/e2e_autonomous/time_corner_model_lap_20260916')
run = root/'codex-time-corner-lap01'
...
baseline_path=root.parent/'time_multiscale_model_lap_20260916/awsim_before.json'
baseline_bytes=baseline_path.read_bytes()
assert hashlib.sha256(baseline_bytes).hexdigest() == '74370d60dc0289d0c5c1d5300476d0cb79cd0200a67075d577ecdcc399b71637'
...
assert before['files']==after, 'AWSIM differs from previous verified baseline'
with (root/'awsim_reference_snapshot.json').open('xb') as stream:
...
with (root/'awsim_preserved.json').open('x') as stream:
    json.dump(dict(status='PASS',files=len(after),bytes=sum(v['bytes'] for v in after.values()),all_files_equal=True,
        scope='POST_RUN_EQUAL_TO_PREVIOUS_VERIFIED_BASELINE',
        baseline_path=str(baseline_path),baseline_sha256=hashlib.sha256(baseline_bytes).hexdigest(),
        fresh_full_pre_run_snapshot_available=False,
        snapshot_omission='package copied the earlier prepare template without full asset snapshot',
        pre_run_critical_asset_hashes=host['steering_asset_sha256']),stream,indent=2)
