...
root = Path('/home/graneple/e2e_autonomous/time_near_limit_diagnostic_20260915')
run = root/'codex-time-near-limit-01'
...
for path in sorted(run.rglob('*')):
    if path.is_symlink():
        ...
        assert target.is_file() and run in target.parents, path
        aliases.append(dict(path=path.relative_to(run).as_posix(),target=target.relative_to(run).as_posix(),sha256=sha(target)))
if aliases:
    with (run/'archive_aliases.json').open('x') as stream:
        json.dump(dict(scope='Internal links omitted; originals preserved and target files included',aliases=aliases),stream,indent=2)
entries=[]
for path in sorted(run.rglob('*')):
...
with tarfile.open(archive,'w:gz') as tar:
    ...
    for entry in entries+[dict(path='transfer_manifest.json')]:
        tar.add(run/entry['path'],arcname=run.name+'/'+entry['path'],recursive=False)
