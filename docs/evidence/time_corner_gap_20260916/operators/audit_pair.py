...
raw=Path('/home/thistle/e2e_autonomous/raw/time_corner_gap_20260916')
proof=json.loads((out/f'cornergap_pair{PAIR:02}_20260916_verified.json').read_text())
...
for domain,split in [(1,'train'),(2,'validation')]:
 name=f'codex-time-recovery-cornergap-p{PAIR:02}-d{domain}'
...
screen=json.loads((out/'candidate_screen.json').read_text())
new_audits=[]
for p in sorted(out.glob('*_collection_summary.json')):
 ...
 new_audits.append(row)
coverage={}
for cm in ('20','40','60'):
 audits=list(new_audits)
 prior_roots=[out.parent/('time_corner_multiscale'+cm+'_20260916')]
 if cm=='20':prior_roots.append(out.parent/'time_corner_recovery_20260916')
 for prior in prior_roots:
  for p in sorted(prior.glob('*_collection_summary.json')):
   row=json.loads(p.read_text());states=prior/(row['run_id']+'_anchor_states.json')
   row['anchor_states']=json.loads(states.read_text()) if row['accepted'] else []
   audits.append(row)
 coverage[cm]=corner_coverage([LargeRecoverySite(**c['site']) for c in screen['catalogs'][cm]['corners']],audits)
report=dict(amplitudes=coverage,new_summary_sha256={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(out.glob('*_collection_summary.json'))})
with (out/f'coverage_after_pair{PAIR:02}.json').open('x') as f:json.dump(report,f,indent=2)
print(json.dumps({cm:dict(complete=c['complete'],missing=c['missing']) for cm,c in coverage.items()}),flush=True)
