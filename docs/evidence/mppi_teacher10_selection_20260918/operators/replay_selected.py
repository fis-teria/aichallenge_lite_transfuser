...
output=ROOT/'collect10_validation_v1'
output.mkdir(exist_ok=False)
selection=json.loads((ROOT/'collect10_curated_v1/selection_manifest.json').read_text())
assert len(selection['runs'])==6, 'Expected six collection conditions'
for name,digest in selection['output_sha256'].items():assert sha(ROOT/'collect10_curated_v1'/name)==digest
summaries=[]
for summary in selection['runs']:
    run=summary['run_id'];selected=ROOT/'collect10_curated_v1'/run;collected=ROOT/'collected'/run
    rowlist=[json.loads(s) for s in (selected/'selected_anchors.jsonl').read_text().splitlines()] if summary['selected'] else []
    coverage=json.loads((ROOT/'collect10_audit_v1'/run/'corner_coverage.json').read_text())
...
(output/'summary.json').write_text(json.dumps(dict(runs=summaries,selection_manifest_sha256=sha(ROOT/'collect10_curated_v1/selection_manifest.json'),
