...
remote((HERE/'seal_archive.py').read_text(), timeout=120)
...
names = ['shipping.json', 'raw_trial.tar.gz', 'post_environment.json', 'runner_start.json', 'runner_exit.json',
         ...
         'build_exit.json', 'smoke_exit.json', 'awsim_before.json', 'awsim_preserved.json']
checks = json.loads(remote('''
from pathlib import Path
import hashlib,json
root=Path('''+repr(ROOT)+''')
names='''+repr(names)+'''
print(json.dumps({name:dict(bytes=(root/name).stat().st_size,sha256=hashlib.sha256((root/name).read_bytes()).hexdigest()) for name in names}))
''', timeout=60))
for name in names:
...
for name, check in checks.items():
    path=native/name
    assert path.stat().st_size == check['bytes'] and hashlib.sha256(path.read_bytes()).hexdigest() == check['sha256'],name
