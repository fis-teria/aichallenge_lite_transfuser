...
root = Path('/home/thistle/e2e_autonomous/runs/time_launch_model_lap_20260917')
raw = root / 'raw/codex-time-launch-lap01'
out = root / 'evaluation'
sources = {}
...
old = root.parent / 'time_multiscale_model_lap_20260916/raw/codex-time-multiscale-lap01'
...
ax.plot(*(xy-origin).T, color='#246cac', linewidth=2, label='Launch-balanced epoch 3: actual trace')
ax.scatter(*(old_xy-origin), marker='X', color='#d48a20', s=100, zorder=5, label='Previous model guard at 208.6 m')
...
ax.set(xlabel='Map X relative to launch [m]', ylabel='Map Y relative to launch [m]',
       title=f"{summary['status']} | Recorded travel {travel:.1f} m | Target 5 km/h")
