...
root=Path('/home/thistle/e2e_autonomous/runs/time_launch_10kmh_stop1m_20260917')
raw=root/'raw/codex-time-launch10-stop1m-lap01'
...
guard=next((r for r in rows if r['event']=='SCAN_GUARD_REJECTED'),None)
if guard is None:
...
n,h,meta=curvature_support_envelope(*motion['curvature_interval_per_m'],travel,scan['angle_increment'],sensor[:2],
    lateral_padding_m=lateral_padding,vehicle_model_policy=AWSIM_10KMH_POLICY)
proof=scan_margin(scan,sensor,speed_mps=speed,measured_rad=guard['measured_steer_rad'],issued_rad=guard['issued_steer_rad'],
    previous_rad=guard['previous_steer_rad'],yaw_rate_radps=obs['heading_rate_radps'],lateral_mps=obs['reported_lateral_mps'],vehicle_model_policy=AWSIM_10KMH_POLICY,stopping_distance_policy=ONE_METRE_STOPPING_TRAVEL)
