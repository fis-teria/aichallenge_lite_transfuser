...
out=base/'runs/time_launch_10kmh_stop1m_20260917';out.mkdir(exist_ok=False)
...
cfg_path=repo/'configs/control/time_path_launch_10kmh_stop1m_20260917.json'
control=json.loads(cfg_path.read_bytes());old=json.loads((repo/'configs/control/time_path_launch_10kmh_stop5_20260917.json').read_bytes())
assert {k for k in control.keys()|old.keys() if control.get(k)!=old.get(k)}=={'stopping_distance_policy'}
...
record=dict(status='PASS',exit=0,source_commit=head,tested_source_commit=head,
    ...
    offline_selection_runtime_test_allowed=False,user_authorization='10 km/h with a 1 m stopping monitor explicitly requested 2026-09-17',
