...
out=base/'runs/time_launch_15kmh_stop1m_20260917';out.mkdir(exist_ok=False)
...
cfg_path=repo/'configs/control/time_path_launch_15kmh_stop1m_20260917.json'
control=json.loads(cfg_path.read_bytes());old=json.loads((repo/'configs/control/time_path_launch_10kmh_stop1m_20260917.json').read_bytes())
assert {k for k in control.keys()|old.keys() if control.get(k)!=old.get(k)}=={'speed_policy','speed_cap_mps','overspeed_limit_mps','vehicle_model_policy'}
assert validate_trial_config(control)=='fixed_15kmh' and control['checkpoint_sha256']==digest
...
record=dict(status='PASS',exit=0,source_commit=head,tested_source_commit=head,
    ...
    offline_selection_runtime_test_allowed=False,user_authorization='15 km/h, then +5 km/h after each passing run, with the inherited 1 m monitor requested 2026-09-17',
