...
out=base/'time_curvature_response_20260917/evaluation'
trials=[('Fixed 10 km/h','time_launch_10kmh_stop1m_20260917','codex-time-launch10-stop1m-lap01'),
        ...
        ('Adaptive + proximity stop','time_curvature_launch_20260917','codex-time-curve15-lap02'),
        ('Adaptive + logging only, reduced response','time_curvature_logonly_20260917','codex-time-curve15-logonly-lap03'),
        ('Adaptive + logging only, original response','time_curvature_response_20260917','codex-time-curve15-response-lap04')]
results=[];fig,axes=plt.subplots(5,1,figsize=(12,13),sharex=False)
new_active=[]
for ax,(label,name,run_id) in zip(axes,trials):
    ...
    result=dict(label=label,source_run=str(raw),lap_completed=summary['judge_lap_confirmed'],
        scan_occupancy_policy=summary.get('scan_occupancy_policy','stop_v1'),
        scan_would_stop_commands=summary.get('scan_would_stop_commands',0),
...
record=dict(scope='FIVE_SINGLE_SAME_SCENE_RUNS_NOT_REPEATED_SUCCESS_RATE',trials=results,
    ...
    proximity_stop_enforcement_differs=True,
