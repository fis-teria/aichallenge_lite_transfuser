...
out=base/'runs/time_curvature_response_20260917';out.mkdir(exist_ok=False)
...
cfg_path=repo/'configs/control/time_path_curvature_logonly_20260917.json'
control=json.loads(cfg_path.read_bytes());old=json.loads((repo/'configs/control/time_path_curvature_preview_20260917.json').read_bytes())
assert {k for k in control.keys()|old.keys() if control.get(k)!=old.get(k)}=={'scan_occupancy_policy'}
assert validate_trial_config(control)=='curvature_preview_15kmh_v1' and control['checkpoint_sha256']==digest
...
baseline=base/'runs/time_launch_10kmh_stop1m_20260917/raw/codex-time-launch10-stop1m-lap01'
...
record=dict(status='PASS',exit=0,source_commit=head,tested_source_commit=head,
    ...
    full_pytest_command=command,full_pytest_exit=0,previous_10kmh_control_replay=baseline_replay['control_replay'],
    checkpoint_sha256=digest,checkpoint_epoch=3,config_sha256=hashlib.sha256(cfg_path.read_bytes()).hexdigest(),
    runtime_contract_valid=True,predictions=rows,runtime_speed_profile_changed=True,longitudinal_response_restored=True,scan_proximity_log_only=True,stopping_distance_diagnostic=True,sealed_test_read=False,
    offline_selection_sha256=hashlib.sha256((prior/'evaluation_final/selection.json').read_bytes()).hexdigest(),
    offline_selection_runtime_test_allowed=False,user_authorization='User explicitly requested an AWSIM experiment with no stopping-monitor region enforcement on 2026-09-17; curvature speed planning retained, proximity recorded only, other guards retained',
