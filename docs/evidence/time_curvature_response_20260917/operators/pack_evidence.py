...
repo = here.parents[1]
native = Path(r'\\wsl.localhost\Ubuntu-22.04-Recovered\home\thistle\e2e_autonomous\runs\time_curvature_response_20260917')
raw = native / 'raw/codex-time-curve15-response-lap04'
out = repo / 'docs/evidence/time_curvature_response_20260917'
...
for path in sorted(here.glob('*.py')):
    if path.name in ('clearance_analysis_native.py','offline_native.py',
                     'stall_native.py','stall_refinement_native.py'):
        continue  # Earlier diagnostic operators not executed for this trial.
