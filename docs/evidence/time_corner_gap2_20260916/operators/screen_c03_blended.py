...
for fraction,bias,heading,ret in itertools.product((.2,.4,.6,.8),(-.06,-.085,-.1),(-.5,0.,.5),(4.,6.)):
 site=replace(original,preparation_origin='blended_normal',preparation_normal_fraction=fraction,
  approach_distance_m=8.,settle_distance_m=4.,return_length_m=ret,entry_window_lead_m=.5,
...
with (out/'c03_blended_screen.json').open('x') as f:json.dump(dict(trials=trials,scope='STATIC_PATH_SCREEN_ONLY'),f,indent=2)
if good:
 choice=min(good,key=lambda r:(abs(r['site']['preparation_normal_fraction']-.5),abs(r['site']['preparation_heading_bias_rad']),abs(r['site']['preparation_offset_bias_m']+.085)))
 with (out/'c03_blended_override.json').open('x') as f:json.dump({'60:C03':choice['site']},f,indent=2)
