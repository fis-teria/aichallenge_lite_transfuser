...
assert args.label.replace('_','').isalnum() or not args.label
label = ('_' + args.label) if args.label else ''
assert 1 <= args.pair <= 6 and 0. <= args.heading_bias_deg <= 1.
...
for split, keys in [('train', args.train), ('validation', args.validation)]:
    ...
    for key in keys:
        ...
        for approach, settle, ret, origin in dict.fromkeys(variants):
            if approach + settle - selected.preparation_delay_m < 6.:
                continue
    ...
    name = f'{split}_p{args.pair:02}_bias{round(args.heading_bias_deg*10):02}{label}'
...
with (OUT / f'adaptive_pair{args.pair:02}{label}.json').open('x') as stream:
