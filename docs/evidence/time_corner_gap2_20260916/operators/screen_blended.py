...
trials=[]
for fraction,bias,heading in itertools.product((.35,.5,.65),(.06,.085,.1),(1.,1.5,2.)):
...
assert valid
choice=min(valid,key=lambda r:(abs(r['site']['preparation_normal_fraction']-.5),abs(r['site']['preparation_offset_bias_m']-.085),abs(r['site']['preparation_heading_bias_rad']-math.radians(1.5))))
with (out/'blended_override.json').open('x') as f:json.dump({'20:C06':choice['site']},f,indent=2)
poses=[]
...
with (out/'blended_and_goal_screen.json').open('x') as f:json.dump(dict(trials=trials,selected=choice,goal_pose_samples=poses,scope='Sampled static poses; no continuous feasibility or dynamic recovery proof'),f,indent=2)
