...
def main() -> None:
    verified = {}
    receipts = {}
    for path in sorted(OUT.glob('corners_pair*_verified.json')):
        receipt = read(path)
    ...
    for name, verification in sorted(verified.items()):
        assert name.startswith('codex-time-recovery-corners-p')
        raw = RAW / name
        ...
        for event_row in row['teacher_anchors_per_event']:
            ...
            event_row['entry_state_anchors'] = sum(
                ...
                and site.at_goal(a['lateral_m'], a['heading_rad']) and 1.15 <= a['speed_mps'] <= 1.4
                for a in states)
        if report['accepted']:
    ...
    catalog_path = Path('configs/collection/corner_recovery_20260916.json')
