...
proof=json.loads((out/f'corners_pair{PAIR:02}_20260916_verified.json').read_text())
...
for domain,split in [(1,'train'),(2,'validation')]:
 name=f'codex-time-recovery-corners-p{PAIR:02}-d{domain}'
...
coverage['catalog_sha256']=hashlib.sha256(Path('configs/collection/corner_recovery_20260916.json').read_bytes()).hexdigest()
