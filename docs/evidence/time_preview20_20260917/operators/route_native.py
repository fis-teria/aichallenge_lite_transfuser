...
old = root.parent / 'time_curvature_launch_20260917/raw/codex-time-curve15-lap02'
...
ax.plot(*(xy-origin).T, color='#246cac', linewidth=2, label='Curvature preview, ceiling 20 km/h: actual trace')
ax.scatter(*(old_xy-origin), marker='X', color='#d48a20', s=100, zorder=5, label='Previous adaptive-speed guard at 203.40 m')
...
ax.set(xlabel='Map X relative to launch [m]', ylabel='Map Y relative to launch [m]',
       title=f"{summary['status']} | Recorded travel {travel:.1f} m | Adaptive speed, ceiling 20 km/h")
