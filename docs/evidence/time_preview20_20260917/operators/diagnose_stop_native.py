...
from aic_transfuser_lite.control.curvature_support_v2 import curvature_support_envelope, stopping_envelope_parameters, ONE_METRE_STOPPING_TRAVEL
from aic_transfuser_lite.control.vehicle_motion_v1 import stopping_motion, AWSIM_20KMH_POLICY
...
raw=root/'raw/codex-time-timepreview20-lap01'
...
guard=next((r for r in rows if r['event'] in ('SCAN_GUARD_REJECTED','SCAN_GUARD_OBSERVED')),None)
if guard is None:
    (out/'diagnosis.json').write_text(json.dumps(dict(status='NO_SCAN_REJECTION',control_sha256=manifest['control.jsonl']['sha256'])))
    print('NO_SCAN_REJECTION');sys.exit(0)
command=next(r for r in rows if r['event']=='COMMAND_SENT' and r['sim_ns']==guard['sim_ns'] and r.get('details',{}).get('current_pose'))
...
motion=stopping_motion(speed,guard['measured_steer_rad'],guard['issued_steer_rad'],guard['previous_steer_rad'],
    policy=AWSIM_20KMH_POLICY,heading_rate_radps=obs['heading_rate_radps'],reported_lateral_mps=obs['reported_lateral_mps'])
sensor=np.asarray(guard['scan_in_current_rear'])
travel,lateral_padding,diagnostic=stopping_envelope_parameters(speed,motion,stopping_distance_policy=ONE_METRE_STOPPING_TRAVEL)
n,h,meta=curvature_support_envelope(*motion['curvature_interval_per_m'],travel,scan['angle_increment'],sensor[:2],
    lateral_padding_m=lateral_padding,vehicle_model_policy=AWSIM_20KMH_POLICY)
proof=scan_margin(scan,sensor,speed_mps=speed,measured_rad=guard['measured_steer_rad'],issued_rad=guard['issued_steer_rad'],
    previous_rad=guard['previous_steer_rad'],yaw_rate_radps=obs['heading_rate_radps'],lateral_mps=obs['reported_lateral_mps'],vehicle_model_policy=AWSIM_20KMH_POLICY,stopping_distance_policy=ONE_METRE_STOPPING_TRAVEL)
...
result=dict(status='PASS',event=guard['event'],action=guard.get('action','STOP'),first_guard_after_arm_s=(guard['sim_ns']-armed)/1e9,current_pose=pose,speed_kmh=speed*3.6,
    ...
    stopping_travel_m=travel,scan_replay=proof,motion=motion,monitor_metadata={**meta,**diagnostic},
    ...
    boundary='First proximity observation reproduced; LOG_ONLY_CONTINUED is not a stop or physical contact measurement.',
