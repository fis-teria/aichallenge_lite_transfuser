"""Plot measured recovery and accepted camera anchors from three actual laps."""
...
NAMES = sorted(p.name.removesuffix('_collection_summary.json') for p in OUT.glob('*-d40-*_collection_summary.json') if json.loads(p.read_text())['accepted'] > 0 and (OUT/(p.name.removesuffix('_collection_summary.json')+'_anchor_states.json')).exists())
...
def main() -> None:
    ...
    for ax, name in zip(axes, NAMES):
        ...
        low, high = sorted([sign * .35, sign * .45])
        ax.axhspan(low, high, color='#dce8f0', alpha=.85, label='35-45 cm target band')
        ...
        for event in report['events']:
            ...
            ax.plot([(r['publication']['sim_ns'] - release) / 1e9 for r in rows],
                    ...
                    color=colors[site], linewidth=1.3, alpha=.7)
        ...
        short = name.removeprefix('codex-time-recovery-40cm-')
