...
result=dict(scope='Observed single model-driven trial with explicitly capped diagnostic stopping horizon',
    host_status=host['status'], target_speed_kmh=10.0, moving_speed_median_kmh=float(np.median(moving)) if moving else None,
...
guards=[]
for row in control:
    if row['event']!='SCAN_GUARD_REJECTED':continue
    ...
    proof=scan_margin(row['scan'], row['scan_in_current_rear'], speed_mps=row['speed_mps'],
        ...
        vehicle_model_policy='awsim_understeer_10kmh_trial_v1',
        stopping_distance_policy='awsim_cap_5kmh_diagnostic_v1')
    assert proof['reason']==row['reason']
    guards.append(dict(sim_after_arm_s=(row['sim_ns']-armed)/1e9,reason=row['reason'],replay=proof))
result['scan_rejections']=guards
guard_details=[r['details']['obstacle_guard'] for r in tracking]
assert all(g['stopping_distance_policy']=='awsim_cap_5kmh_diagnostic_v1' and g['actual_speed_stopping_envelope'] is False for g in guard_details)
result['maximum_recorded_stopping_travel_m']=max(g['stopping_travel_m'] for g in guard_details)
result['stopping_distance_policy']='awsim_cap_5kmh_diagnostic_v1'
