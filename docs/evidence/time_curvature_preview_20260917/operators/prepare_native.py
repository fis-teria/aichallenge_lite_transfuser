...
out=base/'runs/time_curvature_preview_20260917';out.mkdir(exist_ok=False)
...
cfg_path=repo/'configs/control/time_path_curvature_preview_20260917.json'
control=json.loads(cfg_path.read_bytes());old=json.loads((repo/'configs/control/time_path_launch_15kmh_stop1m_20260917.json').read_bytes())
assert {k for k in control.keys()|old.keys() if control.get(k)!=old.get(k)}=={'speed_policy'}
...
record=dict(status='PASS',exit=0,source_commit=head,tested_source_commit=head,
    ...
    runtime_contract_valid=True,predictions=rows,runtime_speed_profile_changed=True,stopping_distance_diagnostic=True,sealed_test_read=False,
    offline_selection_sha256=hashlib.sha256((prior/'evaluation_final/selection.json').read_bytes()).hexdigest(),
    offline_selection_runtime_test_allowed=False,user_authorization='User requested curvature-aware slowing and smooth exit acceleration to avoid walls; 15 km/h ceiling, inherited 1 m monitor, single comparison 2026-09-17',
