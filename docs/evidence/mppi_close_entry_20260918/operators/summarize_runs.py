...
names=['lidar-v45-pc10-front-'+s for s in ['cone-close6-a02','close-entry-a02','close-entry-control-a01','entry-normal-a01']]
summaries=[]
for name in names:
    ...
    summaries.append(dict(run_id=name,early_entry_search=identity.get('early_entry_search',False),
      verdict=result.get('scenario_verdict'),penalties=details['penalty_by_kind'],
...
out=root/'close_entry_comparison.json'
...
print(json.dumps([{k:v for k,v in r.items() if k not in ['runtime_identity','initial_pose','source_manifest_sha256']} for r in summaries],indent=2))
