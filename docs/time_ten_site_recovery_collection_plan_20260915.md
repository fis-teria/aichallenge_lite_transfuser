...
実装・現行正常2周・seed付き地点選定を完了し、有限収集を開始した。
最新の実行状況と検証は [実測収集記録](time_site_recovery_collection_20260915.md) を参照。
以下の「未実施」「必要な変更」は計画作成時点の記録である。
...
計画訂正時点で行ったのは、保存済み正常走行のWSL解析と収集計画の作成だった。
その時点では新しいAWSIM走行、外乱投入、教師収集、再学習は未実施だった。
