...
**最新状態:** 投入位置と継続時間の指摘を受け、group 2左の完走・正常停止で収集を区切った。
後続の自動開始は停止中。[実測確認と修正案](time_recovery_location_duration_review_20260915.md) を参照。
以下は主にgroup 1検証完了時点の記録である。
...
group 2〜4の6run・18予定イベントへ継続を開始し、その後group 2左の3イベントを完了した時点で区切った。
残りの5runは未開始。group 2左のrawはremoteに保全し、まだ教師生成に含めていない。
