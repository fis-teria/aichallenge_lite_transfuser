...
## run16: ユーザー承認の1周shadow試験へ終了条件を変更

40回制限撤廃と再試験の明示指示を受け、TIME_BOUNDEDを追加。
旧COUNT_BOUNDEDは既存契約/試験互換のため維持。新設定ではforward_limit=null、
通常終了はFinish、異常、全体300wall秒（285秒で終了開始）となる。
V4自身も285wall秒/64MiB上限。候補10000はseen集合のメモリ保護であり、
通常10Hzの本試験での終了目標ではない。推論回数は数えるが40で停止しない。
起動から観測を継続、駆動側はStart後最大240sim秒。停止watchdogは実時間。
候補不足やモデル失敗をごまかす閾値変更、速度調整、AWSIM改変は行わない。
