...
- 合格はJudgeの1周完了に加え、走行中の実測速度中央値が目標の90%以上。加速指令だけ上げて実速度が頭打ちの場合は、その段階で終了し制御上の制約を調べる。初回不合格以降の速度は走らせない。
- この90%は今回の速度到達確認の評価基準で、競技の公式基準ではない。制動を含むactive期間の速度サンプルを使い、0.1m/s以下のみ除外する。
