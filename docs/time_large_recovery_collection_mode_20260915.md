...
以下の実装時点の検証記録に加え、[AWSIM実走収集](time_large_recovery_live_20260915.md)を実施した。
左右3回条件は実測教師まで確認済み。5回条件は時計更新の連続性で中断し、成立は未確認。
実走から採用1,094件をWSLへ準備したが、モデルの学習は開始していない。
...
2. 指定した開始位置の1m窓で、最終操舵の入力を準備用PPへ切り替える。
   準備経路は8mで左右へ移動し、`settle_distance_m`の2mまたは4m、目標の横位置を保持する。
...
保持2mの初回例は進捗50mで準備開始、58mから保持、60～61mで復帰要求。
実走で横移動の行き過ぎを確認したため、現行pilot planは保持4mにし、48mで準備開始、
56mから保持、60～61mで復帰要求する。目標判定幅と監視期限は変えていない。
