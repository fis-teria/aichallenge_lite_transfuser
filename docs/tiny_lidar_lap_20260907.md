...
### 短走行後のReady待ち修正（合成確認のみ、追加駆動なし）

`short_4d6ddd2_03` はReady通知が約6.26sim秒に届く前から正加速度要求を開始していた。
操舵/前進/制動停止は実測できたが、8秒枠の大部分は未Readyで、最高速度0.297m/sに留まった。
次版は既存AWSIM `Ready`通知を受け、かつそれ以後に観測したscanのTiny出力があるまで
正加速度を送らない。Ready前の推論は3回で待ち、不要な停車中forwardを増やさない。
これはモデル入力・推論・AWSIM変更ではなく、起動手順の一項目修正。
追加の駆動試験は共通予算の不足を解消する承認待ち。
