...
def assess_speed_step(target_kmh: float, lap_completed: bool, speed_samples_mps: np.ndarray) -> dict[str, Any]:
    """Accept a completed lap only if moving-speed median reaches 90% of target.
    ...
    This is a single-run speed attainment test, not repeatability or safety.
    """
    ...
    status = 'PASS' if lap_completed and reached else ('LAP_NOT_COMPLETED' if not lap_completed else 'TARGET_SPEED_NOT_REACHED')
    return dict(status=status, target_kmh=target_kmh, lap_completed=lap_completed,
                ...
                minimum_accepted_median_kmh=threshold, target_speed_reached=reached,
