...
def preparation_course(base: Sequence[MpcReferencePointV3], normal: np.ndarray,
    ...
    is not necessarily commanded a second time. A blended origin interpolates
    command x/y between the original path and measured normal trace using a
    dimensionless fraction in [0,1]. It does not interpolate goal frames or
    observed teachers. Actual poses remain labels.
    ...
    for site in config.sites:
        ...
        if site.preparation_origin == 'nominal_path':
            ...
            if site.preparation_origin == 'blended_normal':
                fraction = site.preparation_normal_fraction
                nominal_x, nominal_y = np.interp(progress, bs, bx), np.interp(progress, bs, by)
                nx = nominal_x + fraction*(nx-nominal_x)
                ny = nominal_y + fraction*(ny-nominal_y)
