"""Explicit, bounded AWSIM-only V4-10 controller with independent sensor watchdog.
...
def main() -> None:
    ...
    rclpy.init()
    node=Node('v4_10_controller',enable_rosout=False,start_parameter_services=False)
    ...
    def tick():
        ...
        if auth.exists() and state['armed_at'] is None:
            ...
            if value.get('scope')!='V4_10_AWSIM_TRACKING' or wall>value['expires_monotonic_s']:
        ...
        try:
            ...
            if elapsed is not None and elapsed>=10:
                ...
                if p.get('event')!='PLAN' or p.get('source')!='TEN_V4_10M_EPOCH2_UNCORRECTED':raise ValueError('PLAN_IDENTITY')
                ...
                details=tracking_command(p['raw_xy_m'],tuple(p['observation_pose_xyyaw']),current,speed,expected_points=36)
