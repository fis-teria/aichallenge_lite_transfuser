...
def main() -> None:
    ...
    study = ROOT / 'refinement'; study.mkdir(exist_ok=True)
    path = study / 'state.json'
    previous = json.loads((ROOT / 'search/state.json').read_text())
    if not previous['completed']:
        raise RuntimeError('The preceding CMA search must be complete')
    if path.exists():
        state = json.loads(path.read_text())
        if state['completed']:
            print('Refinement already completed', flush=True); return
        ...
        state = {'study_kind': 'refinement', 'mode': 'independent', 'start_mode': 'd1',
                 ...
                 'deadline_unix_s': time.time() + 5400, 'maximum_new_episodes': 64,
                 ...
                 'previous_search_sha256': hashlib.sha256((ROOT / 'search/state.json').read_bytes()).hexdigest()}
        for condition in ('normal', 'leader'):
            incumbent = previous['conditions'][condition]['best']
            state['conditions'][condition] = {'incumbent': incumbent, 'next_generation': 0, 'evaluations': []}
    ...
    def reserve(count: int) -> None:
        if time.time() > state['deadline_unix_s'] or state['new_episodes_started'] + count > 64:
            raise RuntimeError('Finite refinement budget reached')
    ...
    def measurements(condition: str, jobs: list[dict], group_name: str) -> list[dict]:
        ...
        def started(_index: int, job: dict) -> None:
            reserve(1); state['active_episodes'].append(job['name'])
        ...
        for condition, progress in state['conditions'].items():
            ...
            for generation in range(progress['next_generation'], 3):
                ...
                for index, anchors in enumerate(proposals):
                    name = f'r2-{condition}-g{generation:02d}-c{index:02d}'
                ...
                for batch in range(2):
                    ...
                    measured = measurements(condition, subset, f'r2-{condition}-g{generation:02d}-b{batch}')
