    ...
    for condition,baseline in [('normal','baseline-normal02'),('leader','baseline-leader01')]:
        ...
        for generation in range(progress['next_generation'],3):
            ...
            for index,anchors in enumerate(proposals):
                ...
                metrics=evaluate(name,condition,candidate);objectives.append(metrics['objective'])
                record={'metrics':metrics,'anchors_m':anchors,'reference':str(candidate)}
                if not any(x['metrics']['episode']==name for x in progress['evaluations']):progress['evaluations'].append(record)
        ...
        best=min(progress['evaluations'],key=lambda x:x['metrics']['objective'])
        progress['best']=best;state['phase']='validation';save(state_file,state)
        repeats=[]
        for i in range(2):
            repeats.append(evaluate(f'{condition}-best-repeat{i+1}',condition,Path(best['reference'])))
        repeated_baseline=evaluate(condition+'-baseline-repeat1',condition,ROOT/'snapshot/base_reference.csv')
