...
def main() -> None:
    ...
    result = {"status": "COMPLETED_NO_POSITIVE_DRIVE" if positive == 0 else "COMPLETED_DRIVE_OBSERVED",
        ...
        "boundary": "Near-origin heading gate rejected predictions; no measured lane-tracking accuracy or model teacher error.",
    ...
    axes[1].annotate("0.1 s", example[0], xytext=(8, 8), textcoords="offset points")
    ...
    fig.suptitle("Proposed time model: all authorized commands braked for TIME_PATH_FOLDBACK" if positive == 0
                 else "Proposed time model: bounded AWSIM trial")
