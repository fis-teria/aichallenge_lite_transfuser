...
project='codex-pp-reference-lap-12'
...
env=dict(os.environ,DISPLAY=':0',XAUTHORITY='/run/user/1000/.mutter-Xwaylandauth.H56RU3',
         COMPOSE_PROJECT_NAME=project,COMPOSE_FILE=str(repo/'docker-compose.yml')+':'+str(repo/'docker-compose.gpu.yml')+':'+str(override),CONTROL_METHOD='pure_pursuit',V4_SHADOW_ENABLED='false',
...
prior=AttemptBudget(Path('/home/graneple/e2e_autonomous/mpc_guard_lap_11/budget.json'))
assert prior.value['active'] is None
reservation=dict(wall_s=120.,forward=0,mpc=0,snapshots=0,powered=1,powered_s=90.,log_bytes=16*1024**2)
new_budget=root/'budget.json'
assert not new_budget.exists()
renewal=dict(id=project,source='User requested one AWSIM PP test; wall120s, post-Start90sim s, PP commands<=60000; MPC/V4 OFF',unix_s=time.time(),
...
try:
    ...
    cmd=['make','dev','CONTROL_METHOD=pure_pursuit','CAPTURE=false','ROSBAG=false','AWSIM_LAPS=1',
         'RUN_ID=pp_reference_lap_12','OUTPUT_HOST_ROOT='+str(out)]
    result['command']=cmd
