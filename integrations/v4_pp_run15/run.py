...
prior=AttemptBudget(Path('/home/graneple/e2e_autonomous/v4_pp_shadow_14/budget.json'))
    ...
    while not done.wait(.05):
        ...
        if p.exists():
            try:
                ...
                log=out/'v4_pp_shadow_15/d1/autoware.log'
                if log.exists():
...
try:
    ...
    cmd=['make','dev','CONTROL_METHOD=pure_pursuit','CAPTURE=false','ROSBAG=false','AWSIM_LAPS=1',
         'RUN_ID=v4_pp_shadow_15','OUTPUT_HOST_ROOT='+str(out)]
    result['command']=cmd
