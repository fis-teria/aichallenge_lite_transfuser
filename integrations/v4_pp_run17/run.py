"""Bounded V4 sim-clock shadow regression with unchanged PP driving."""
...
prior=AttemptBudget(Path('/home/graneple/e2e_autonomous/v4_pp_shadow_16/budget.json'))
assert prior.value['active'] is None
reservation=dict(wall_s=300.,forward=10000,mpc=0,snapshots=0,powered=1,powered_s=240.,log_bytes=128*1024**2)
new_budget=root/'budget.json'
assert not new_budget.exists()
renewal=dict(id=project,source='User approved removing forty-forward normal termination; one lap shadow with 300wall/240drive seconds; forward reservation is memory-budget accounting, PP alone controls',unix_s=time.time(),
...
live['envelope'].update(session_id=project,authorized_until_unix_s=time.time()+600)
...
def watch():
    while not done.wait(.05):
        reason=None
        if time.monotonic()-start>285: reason='HOST_WALL_LIMIT'
        # Observe V4 shutdown even while make is waiting for its Start helper.
        shadow=out/'shadow.jsonl'
        if shadow.exists():
            with shadow.open('rb') as stream:
                stream.seek(max(0,shadow.stat().st_size-8192))
        ...
        if p.exists():
            try:
                ...
                if result.get('start_requested'):
                    if s.get('sim_s',0)-result['start_sim_s']>=238: reason='DRIVE_TIME_LIMIT'
                    if time.monotonic()-result['start_monotonic']>=245: reason='START_WALL_LIMIT'
                    if (out/'v4.done').exists(): reason='V4_ENDED'
                # Preserve the existing failure-stop rule used manually in run 09.
                log=out/'v4_pp_shadow_17/d1/autoware.log'
                if log.exists():
...
try:
    ...
    cmd=['make','dev','CONTROL_METHOD=pure_pursuit','CAPTURE=false','ROSBAG=false','AWSIM_LAPS=1',
         'RUN_ID=v4_pp_shadow_17','OUTPUT_HOST_ROOT='+str(out)]
    result['command']=cmd
