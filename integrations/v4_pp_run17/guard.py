...
import time
import math
from command_monitor import command_fault
...
last={}
# Verified PP launch output-domain bound; raw is requested post-gain, pre-limit.
limits={'raw_limit_rad':0.64,'gain':1.0}
...
def receive(role,m):
    ...
    if role=='clock':
        ...
        if s['sim_s']>285: s['fault']='TOTAL_SIM_TIME_LIMIT'
    elif role=='ego':
        ...
        fault=None if all(math.isfinite(v) for v in s['command']) else 'RAW_COMMAND_NONFINITE'
        if fault: s['fault']=fault
    elif role=='final':
        s['final_command']=[m.lateral.steering_tire_angle,m.longitudinal.speed,m.longitudinal.acceleration]
        fault=command_fault(s['final_command'],limits['raw_limit_rad'],limits['gain'],final=True)
        if fault: s['fault']=fault
        if m.longitudinal.acceleration>3.0: s['fault']='ACCELERATION_INPUT_LIMIT'
        s['max_acceleration_mps2']=max(s.get('max_acceleration_mps2',0),m.longitudinal.acceleration)
...
try:
    while time.monotonic()-start<292 and s['fault'] is None:
        ...
        if s['ready']:
            ...
            if len(endpoints)!=1 or endpoints[0].node_namespace.rstrip('/')+'/'+endpoints[0].node_name!='/simple_pure_pursuit_node':
