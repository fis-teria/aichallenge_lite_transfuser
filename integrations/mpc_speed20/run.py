"""One authorized MPC 20 km/h make-dev comparison; exact-project shutdown."""
...
project='codex-mpc-speed20-lap-10'
...
override.write_text(json.dumps({'services':{'autoware':{'volumes':[
    str(root)+':/v4',str(root/'config.yaml')+':'+config_target+':ro']}}}))
env=dict(os.environ,DISPLAY=':0',XAUTHORITY='/run/user/1000/.mutter-Xwaylandauth.H56RU3',
         COMPOSE_PROJECT_NAME=project,COMPOSE_FILE=str(repo/'docker-compose.yml')+':'+str(repo/'docker-compose.gpu.yml')+':'+str(override),CONTROL_METHOD='mpc',V4_SHADOW_ENABLED='false',
...
prior=AttemptBudget(Path('/home/graneple/e2e_autonomous/mpc_normal_lap_09/budget.json'))
assert prior.value['active'] is None
reservation=dict(wall_s=120.,forward=0,mpc=60000,snapshots=0,powered=1,powered_s=90.,log_bytes=16*1024**2)
new_budget=root/'budget.json'
assert not new_budget.exists()
renewal=dict(id=project,source='User approved additional one run: wall 120s, post-Start 90sim s, MPC 60000; v_max 20 km/h, V4 OFF',unix_s=time.time(),
    ...
    while not done.wait(.05):
        ...
        if p.exists():
            try:
                ...
                log=out/'mpc_speed20_lap_10/d1/autoware.log'
                if log.exists():
                    tail=log.read_text(errors='replace')[-32000:]
                    import re
                    counts=re.findall(r'mpc_infeasible=(\d+)',tail)
                    if counts and int(counts[-1])>=20: reason='REPEATED_MPC_FAILURE'
...
try:
    ...
    result.update(start_requested=False,start_sim_s=0.,start_monotonic=time.monotonic(),
                  start_basis='NORMAL_DEV_LAUNCH_NOT_START_PULSE')
    (out/'armed.json').write_text(json.dumps(dict(project=project,start_sim_s=0.)))
    cmd=['make','dev','CONTROL_METHOD=mpc','CAPTURE=false','ROSBAG=false','AWSIM_LAPS=1',
         'RUN_ID=mpc_speed20_lap_10','OUTPUT_HOST_ROOT='+str(out)]
    result['command']=cmd
