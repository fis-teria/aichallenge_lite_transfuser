...
project='codex-mpc-guard-lap-11'
...
prior=AttemptBudget(Path('/home/graneple/e2e_autonomous/mpc_speed20_lap_10/budget.json'))
...
renewal=dict(id=project,source='User approved unused additional one run: wall 120s, post-Start 90sim s, MPC 60000; guarded MPC, v_max 20 km/h, ay_max 3 m/s2, V4 OFF',unix_s=time.time(),
    ...
    while not done.wait(.05):
        ...
        if p.exists():
            try:
                ...
                log=out/'mpc_guard_lap_11/d1/autoware.log'
                if log.exists():
...
try:
    ...
    cmd=['make','dev','CONTROL_METHOD=mpc','CAPTURE=false','ROSBAG=false','AWSIM_LAPS=1',
         'RUN_ID=mpc_guard_lap_11','OUTPUT_HOST_ROOT='+str(out)]
    result['command']=cmd
