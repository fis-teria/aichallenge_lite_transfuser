...
ifeq ($(CONTROL_METHOD),tiny_lidar_net_guarded)
	python3 tools/tiny_dev_runner.py --control-method tiny_lidar_net_guarded --sim-repo "$(SIM_REPO)" --official-package "$(TINY_PACKAGE)" --install-root "$(TINY_INSTALL)" --display "$(DISPLAY)" --xauthority "$(XAUTHORITY)" --output "$(TINY_OUTPUT)" --commit "$(TINY_COMMIT)" --phase short --wall-seconds 120 --budget "$(TINY_BUDGET)"
else
